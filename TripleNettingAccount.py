...
class TripleNettingAccount():
    def __init__(self, all_money:float, unit=None, stop_profit=None):
        ...
        self.trade_records = pd.DataFrame(columns=['side', 'trade_type', 'trade_type_desc', 'trade_strategy',
                                                   'tradetime', 'middleline', 'atr5', 'tradecount', 'tradeprice', 'trademoney'])
        ...
        self.position = pd.DataFrame(columns=['side', 'positioncnt', 'avgprice', 'middleline', 'atr5'])
    ...
    def order_all(self, trade_time:str,trade_type:str, trade_strategy:str, side:str, price:float, middleline:float, atr5:float):
        ...
        self.trade_records.loc[len(self.trade_records)] = [side, trade_type, self.all_money, trade_strategy,
                                                               trade_time, middleline, atr5,
                                                               position_cnt, price, price * position_cnt]
        #刷新最新持仓
        self.position.loc[0] = [side, position_cnt, price, middleline, atr5]
    ...
    def close_oder(self, trade_time:str,trade_type:str,trade_strategy:str, side:str, price:float,all_money:float,position_cnt:int,position_all_money:float, middleline:float, atr5:float):
