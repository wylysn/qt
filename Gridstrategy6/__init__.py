...
while True:
    ...
    if n_loc is not None:
        ...
        if len(ac.singleloop_trade_records) > 0 and flag == 0:
            ...
            # 以初始买入价格A1为基准
            will_buy_price = init_tradeprice + 80
            if add_position_count == 0:
                will_buy_price = init_tradeprice + 80
            elif add_position_count == 1:
                will_buy_price = init_tradeprice + 140
            elif add_position_count == 2:
                will_buy_price = init_tradeprice + 175
            elif add_position_count == 3:
                will_buy_price = init_tradeprice + 200
            elif add_position_count == 4:
                will_buy_price = init_tradeprice + 230
            ...
            if n_avgprice >= will_buy_price and len(ac.position) > 0:
                ...
                if add_position_count < 3:
                    ...
                    trade_count = trade_count_unit * 6
                else:
                    log.info('最多加仓5次')
                    add_position_flag = 0
